...
export function reject<Input>(fn: Predicate<Input>, arrOrObj?: List<Input>): List<Input> | Curried<Input, Input> {
  const innerFilter = (arrOrObj: List<Input>) => {
    ...
    else throw new Error(`goated.reject() only accepts arrays or objects. Received ${arrOrObj}`)
  }
