...
export function filter<Input>(fn: Predicate<Input>, arrOrObj?: List<Input>): List<Input> | Curried<Input, Input> {
  const innerFilter = (arrOrObj: List<Input>) => {
    ...
    else throw new Error(`goated.filter() only accepts arrays or objects. Received ${arrOrObj}`)
  }
