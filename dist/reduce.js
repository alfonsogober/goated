...
function reduce(fn, initialElement, arrOrObj) {
    const innerReduce = (arrOrObj) => {
        if (arrOrObj instanceof Array) {
            ...
            return keys_1.keys(arrOrObj).reduce((prev, curr, idx) => fn(prev, arrOrObj[curr], idx, arrOrObj), initialElement);
        }
        else
            throw new Error(`goated.reduce() only accepts arrays or objects. Received ${arrOrObj}`);
    };
